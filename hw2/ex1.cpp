...
int main()
{
	...
	double first_num;
    double second_num;
	cout << "Нужно ли использовать значение по модулю? (1 - да, 0 - нет 0) " << endl;
	cin >> use_module;
	...
	if (operation == "+") {
		cout << "Вы произвели операцию сложение, " 
			 << module_first << " + " << module_second << " = " << module_first + module_second;
		check_module(use_module);
	}
	else if (operation == "-") {
		...
		cout << "Вы произвели операцию нахождение процента от числа, "
			<< module_first << " * " << module_second << "% " << " = " << module_first / 100 * module_second;
		check_module(use_module);
	}
